...
| AI Dev Claude | 0.1.0 | `helpers/ai-dev-claude/ai-dev-claude.sh` | Creates and provisions a new LXC for Claude Code development: web IDE (code-server), web file manager (FileBrowser Quantum), Termix, a Homepage link dashboard, GitHub CLI, and Claude Code, with a `--yes` unattended mode and an OmniRoute-connect option. `ai-dev-claude-no-docker.sh` in the same helper is a Docker-free variant (ttyd + a static dashboard instead of Termix/Homepage) |
