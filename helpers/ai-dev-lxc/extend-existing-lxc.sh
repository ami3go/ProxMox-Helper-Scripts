...
#
# Extends/repairs an AI Development LXC that already exists (created by
# install.sh). Sources lib/*.sh and does the actual Codex/Homepage/Caddy
# provisioning; install.sh handles creating the container from scratch.
...
usage() {
  cat <<'USAGE'
AI Development LXC Gateway/Codex extend/repair tool v2.2.7

Usage:
  extend-existing-lxc.sh                         Interactive; auto-detect Proxmox host or LXC
  extend-existing-lxc.sh --ctid 123              Configure an existing LXC from Proxmox host
  extend-existing-lxc.sh --guest                 Provision from inside the LXC
  extend-existing-lxc.sh --guest --state FILE    Provision from a supplied state file
  extend-existing-lxc.sh --non-interactive       Use state/environment defaults without prompts
  extend-existing-lxc.sh --validate-only         Validate the loaded state and exit
  extend-existing-lxc.sh --version               Print the helper version and exit
...
host_push_and_run() {
  ...
  pct exec "$CTID" -- /opt/ai-dev-lxc-helper/extend-existing-lxc.sh --guest --non-interactive
}
