...
#
# Run this script as root on the Proxmox VE host. Self-contained; does not
# source lib/*.sh. Creates a new LXC from scratch and provisions it — see
# extend-existing-lxc.sh for the separate tool that repairs/updates an
# AI Development LXC that was already created by this script.

set -Eeuo pipefail
shopt -s inherit_errexit 2>/dev/null || true

case "${1:-}" in
  --version|-v) printf '2.2.7\n'; exit 0 ;;
  --help|-h)
    printf 'Usage: %s\n\nInteractive; run as root on the Proxmox VE host to create and provision\na new AI Development LXC, or repair/inspect one this script created.\n' "$(basename "$0")"
    exit 0
    ;;
esac
